...
**Active strategy + branch policy = the `git_strategy:` block in `.agents/project.yaml`** (source of truth; see `## Git Strategy` below). This repo operates as `github-flow`.
...
| Branch | Status | Role |
|---|---|---|
| `chore/BK-configure-agents-project-yaml` | Always | Production branch (origin/HEAD). NOT a literal `main` — local `main` exists but is unused. Every change is a short-lived prefixed branch (`docs/*`, `test/*`, `chore/*`, `feat/*`) merged via PR; work also lands by DIRECT push under this repo's `allowed` policy (see `## Git Strategy`). |
| `staging` | Not used | This repo has no integration branch. Do not assume one exists. |
